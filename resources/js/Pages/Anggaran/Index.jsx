...
import { CheckCircleIcon, MinusCircleIcon, PencilSquareIcon, TrashIcon, PlusIcon, WalletIcon, DocumentTextIcon } from '@heroicons/react/24/solid';
...
export default function Index({ auth, anggaranData, isAdmin }) {
    ...
    const toggleActive = (id) => {
        router.post(route('anggaran.toggle_active', id), {}, {
            preserveScroll: true
        });
    };

    const renderToggleSwitch = (item) => {
        const isActive = item.is_active;
        return (
            <button 
                type="button"
                onClick={() => isAdmin ? toggleActive(item.id) : null}
                className={`${isActive ? 'bg-amber-400' : 'bg-gray-200'} relative inline-flex h-6 w-11 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none ${!isAdmin && 'opacity-70 cursor-not-allowed'}`}
                disabled={!isAdmin || processing}
                aria-pressed="false"
            >
                <span className="sr-only">Use setting</span>
                <span aria-hidden="true" className={`${isActive ? 'translate-x-5' : 'translate-x-0'} pointer-events-none inline-block h-5 w-5 transform rounded-full bg-white shadow ring-0 transition duration-200 ease-in-out`}></span>
            </button>
        );
    };
                                ...
                                <table className="w-full text-left border-collapse">
                                    <thead>
                                        <tr className="bg-blue-900 text-white font-black uppercase text-[11px] border-b border-blue-950 tracking-[0.2em] italic">
                                            ...
                                            <th className="px-10 py-8 text-center border-l border-blue-950/30 text-[10px]">STATUS</th>
                                            {isAdmin && <th className="px-10 py-8 text-center text-[10px]">AKSI</th>}
                                        </tr>
                                    </thead>
                                    <tbody>
                                        {anggaranData?.map(parent => (
                                            <React.Fragment key={parent.id}>
                                                <tr className={`border-b border-gray-100 group ${!parent.is_active ? 'bg-red-50/10 opacity-60 grayscale' : 'bg-gray-50'}`}>
                                                    <td className="px-10 py-10 border-r border-gray-50 relative">
                                                        <div className={`absolute left-0 top-0 bottom-0 w-2 ${parent.is_active ? 'bg-amber-400' : 'bg-gray-300'}`}></div>
                                                        <div className="flex items-center">
                                                            ...
                                                            <span className={`font-mono font-black text-base italic tracking-tighter ${parent.is_active ? 'text-blue-900' : 'text-gray-500 line-through'}`}>'{parent.kode}</span>
                                                        </div>
                                                    </td>
                                                    <td className="px-10 py-10 border-r border-gray-50">
                                                        <p className={`font-black uppercase leading-snug tracking-tighter italic mb-2 text-base ${parent.is_active ? 'text-gray-800' : 'text-gray-400 line-through'}`}>{parent.nomenklatur}</p>
                                                        <div className="inline-flex items-center bg-white px-4 py-1.5 rounded-full border border-gray-200">
                                                            <DocumentTextIcon className={`h-4 w-4 mr-2 ${parent.is_active ? 'text-amber-500' : 'text-gray-400'}`} />
                                                    ...
                                                    <td className="px-10 py-10 text-center border-l border-gray-50">
                                                        {renderToggleSwitch(parent)}
                                                    </td>
                                                    {isAdmin && (
                                                ...
                                                {expandedRows[parent.id] && parent.children?.map(child => (
                                                    <tr key={child.id} className={`border-b border-gray-50 transition-all group/child ${!child.is_active ? 'bg-red-50/5 opacity-60 grayscale' : 'bg-white hover:bg-blue-50/20'}`}>
                                                        <td className={`px-10 py-8 border-r border-gray-50 pl-24 font-mono font-black text-sm italic ${child.is_active ? 'text-gray-400' : 'text-gray-300 line-through'}`}>'{child.kode}</td>
                                                        <td className="px-10 py-8 border-r border-gray-50">
                                                            <p className={`font-black text-sm uppercase tracking-tighter mb-1 ${child.is_active ? 'text-gray-600' : 'text-gray-400 line-through'}`}>{child.nomenklatur}</p>
                                                        ...
                                                        <td className={`px-10 py-8 border-r border-gray-50 text-right font-bold text-[13px] ${child.is_active ? 'text-blue-800' : 'text-gray-400'}`}>{formatRp(child.anggaran_realisasi)}</td>
